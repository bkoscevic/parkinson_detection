...
df_train_images = pd.DataFrame({'path': list(data_dir.glob(r'spiral/training/augm_*/*.png'))})
...
plot_model(model, to_file='spiral/model_spiral_plot_VR.png', show_shapes=True, show_layer_names=True, rankdir='TB')

tensor_board = TensorBoard(log_dir='logs', histogram_freq=0, write_graph=True, write_images=True)
log_file_path = 'logs/spiral_training_best.log'
csv_logger = CSVLogger(log_file_path, append=False)
reduce_lr = ReduceLROnPlateau(
    monitor='val_loss', factor=0.1, patience=10, verbose=1, mode='auto',
    min_delta=0.001, cooldown=0, min_lr=0)
trained_models_path = 'models/best_model_spiral.hdf5'
